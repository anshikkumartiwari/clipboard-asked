...
const prompt0 = ``; /* INSERT PROMPT FOR MODE 0 for egample prompt to write c code*/
const prompt1 = ``; /* INSERT PROMPT FOR MODE 1 for exas,ple prompt to write java code */

function extractCode(response, mode) {
  const codeRegex = mode === 'mode0' ? /```c(.*?)```/s : /```java(.*?)```/s;
...
function sendToGeminiAPI(inputText, mode) {
  ...
  const selectedPrompt = mode === 'mode1' ? prompt1 : prompt0;
